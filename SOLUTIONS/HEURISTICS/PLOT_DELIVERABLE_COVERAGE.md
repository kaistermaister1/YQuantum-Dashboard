...
|----------|-----------------|-------------------|
| `plots/heuristics_summary*.png`, `plot_visualizations.py heuristics-summary` | profit vs classical, runtime, feasibility | Quality + cost + constraint handling (1–2) |
| `plots/algorithm_comparison_*.png`, `plot_visualizations.py algorithm-comparison` | side-by-side profit, runtime, shots | Method comparison (3) |
| `plots/profit_ratio_vs_n*.png`, `plot_visualizations.py profit-ratio-vs-n` | approximation ratio vs `N_local` | Quality scaling (1) |
| `plots/runtime_vs_n_total*.png`, `plot_visualizations.py runtime-vs-n-total` | runtime vs `n_total`, eval counts | Time-to-solution proxy vs size (2) |
| `plots/runtime_vs_n_classical_qaoa*.png`, `plot_visualizations.py runtime-vs-n` | classical vs QAOA runtime | Classical baseline (2–3) |
...
| **`plot_visualizations.py runtime-vs-m-blocks`** | same data selection; matplotlib styling for quick iteration | Same as above (2) |
