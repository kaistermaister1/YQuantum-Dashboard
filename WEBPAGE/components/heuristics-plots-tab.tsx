...
export const HEURISTICS_PLOT_PUBLIC_PATHS = {
  ...
  runtime_vs_m_proportional: "/plots/heuristics/runtime_vs_m_proportional_budget.png",
  ...
  simulation_coverage: "/plots/heuristics/simulation_coverage_by_nmp.png",
  run_summary_metrics: "/plots/heuristics/run_summary_aggregate_metrics.png"
} as const;
...
export const HEURISTICS_PLOTS: DashboardPlot[] = [
  ...
  {
    id: "runtime_vs_m_proportional",
    category: "Workflow Cost",
    title: "Runtime vs Packages (Controlled Budget)",
    subtitle: "Wall time vs M_blocks with shots ∝ M (CSV-backed)",
    src: HEURISTICS_PLOT_PUBLIC_PATHS.runtime_vs_m_proportional,
    alt: "Scatter plot of QAOA wall time vs number of packages with proportional shot budget and linear fit.",
    summary: [
      "With a fixed shot budget per package, wall time tracks M_blocks nearly perfectly linearly."
    ],
    description: [
  ...
  },
  {
    ...
    category: "Workflow Cost",
    title: "Simulation coverage (table)",
    subtitle: "Unique (N_local, M_blocks, p) and methods in run_summaries.csv",
    src: HEURISTICS_PLOT_PUBLIC_PATHS.simulation_coverage,
    alt: "Table of problem shapes and which algorithms ran for each.",
    summary: [
      "One-row-per-(n,m,p) view of what was actually simulated."
    ],
    description: [
      "Generated by generate_plots.py from HEURISTICS/run_summaries.csv; shows which classical and QAOA configurations exist for each problem shape."
    ]
  },
  {
    id: "run_summary_metrics",
    category: "Workflow Cost",
    title: "Aggregate metrics (table)",
    subtitle: "Mean wall time by method; local QAOA quality & feasibility",
    src: HEURISTICS_PLOT_PUBLIC_PATHS.run_summary_metrics,
    alt: "Two-part table of wall-time statistics and summary metrics from run_summaries.csv.",
    summary: [
      "Slide-ready averages: wall time by bucket, classical vs local QAOA, approximation ratio, feasibility."
    ],
    description: [
      "Top: mean, median, min, max runtime_sec for each method bucket (classical, local QAOA + COBYLA/SPSA, Selene, etc.).",
      "Bottom: row counts, local QAOA mean/median wall time by optimizer, profit ratio vs classical optimum, feasibility rate, classical ILP timing, and repeated classical baseline for n=10, m=3. Regenerate with generate_plots.py."
    ]
  },
  {
    id: "evals",
