...
Examples:
  ...
  python scripts/benchmark_dqi_official_sizes.py --execution local --skip-ns 200
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--execution",
        default="local",
        help="DQI backend: local, selene, nexus_selene, nexus_helios (see dqi_backends)",
    )
...
def main() -> int:
    ...
    for n in ns:
        ...
        if "bruteforce" in bench:
            ref_val = float(bench["bruteforce"].best_value)
    ...
    plot_scaling_benchmark(
        ...
        out_path=plot_path,
        title_prefix="Official sizes benchmark",
    )
    print("\nwrote:", plot_path)
