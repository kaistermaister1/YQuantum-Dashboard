...
def make_cim_matrix(problem) -> np.ndarray:
    ...
    for package_index in range(problem.M):
        ...
        for coverage_index, coverage in enumerate(problem.coverages):
            affinity = float(problem.get_affinity(coverage_index, package_index))
...
def build_package_local_positive_ilp(
    ...
    objective_expanded = np.concatenate(
        (objective_int.astype(int), np.zeros(len(negative_list), dtype=int)),
    )
